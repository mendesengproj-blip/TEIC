% PAPER_PHOTON_ARC_CQG --- standalone article targeting Classical and Quantum
% Gravity (informative-negative result, the venue's stated remit).
%
...
% converts to the iopart (CQG) class; the revtex build here is for review/length.
%
...
\title{Where, and why, a photon is hard to emerge in causal set theory:\\
a systematic map and an indefinite-signature gauge operator}
...
\begin{abstract}
A recurring hope in emergent-spacetime programmes is that the photon---a massless
transverse gauge vector with a Coulomb phase---should arise from the substrate rather
than be postulated. We report a systematic, pre-registered search for such a photon on
a causal set (a Poisson sprinkling with its causal order), and the result is an
informative negative: a map of exactly where and why the photon is hard, ending at a
precisely stated open construction. Six measured links. (i)~The Goldstone modes of an
internal orientation order are two degenerate internal \emph{scalars}, not a gauge
vector: their polarisation does not lock to the wavevector (permutation $p=0.23$); a
$0$-form Goldstone of a global internal symmetry is structurally scalar. (ii)~We
relocate the question to the gauge-connection (link $U(1)$) sector, where the engine is
validated but the bare connection inherits the causal set's non-locality, and the
confinement/Coulomb phase shows a lattice-like crossover near $\beta\simeq1$ (Coulomb
not certified). (iii)~We construct an indefinite-signature ($E^2-B^2$)
Benincasa--Dowker gauge operator---absent from the causal-set literature, which carries
the d'Alembertian only on scalars---which is gauge-invariant to $5.7\times10^{-16}$ and
reproduces free Maxwell on a $4$D lattice ($\omega=ck$, $c=1.05$); but on the causet it
fails, because causal diamonds are $100\%$ electric (magnetic-cell fraction
$\fracB=0.0000$): the magnetic sector is structurally empty. (iv)~Diamond \emph{height}
does not supply the missing spacelike cell (tail $\sim0.25\%$, non-growing).
(v)~Spatial \emph{curvature} does: $\fracB$ rises monotonically and crosses $0.01$ at de
Sitter radius $\Rhat=2$. (vi)~Coupling the ordered ferromagnet to the gauge field does
not amplify it (the rise is disorder, not order). Extrapolating the one positive lever,
$\Delta\fracB\propto(1/\Rhat)^{1.73}\approx H^2$ ($R^2=0.997$), an $O(10\%)$ magnetic
sector requires $\Rhat\approx0.5$--$0.6$---\emph{sub-Planckian}. The photon mechanism is
therefore real but Planck-scale, and does not explain the photons of the
observable low-curvature universe. (vii)~Two further low-curvature levers---future-cone
cells and a genuine Bianchi-I anisotropy on the causal \emph{order}---are exhausted as well
(the former gauge-trivial and off the Hasse-link substrate, the latter redistributing but
not magnetising), closing the low-curvature map. We state the remaining obstruction exactly:
a Lorentz-invariant spacelike $2$-cell on the causal order at low curvature.
\end{abstract}
...
This paper is deliberately a negative result, in the sense \emph{Classical and Quantum
Gravity} explicitly welcomes: a clear map of where a natural construction fails and why,
ending at an open problem stated precisely enough to be attacked. It is also the
...
CST literature (\S\ref{sec:cst}), and delimit precisely what is and is not established
(\S\ref{sec:claim}).
...
finite-volume Lorentz-invariant local neighbourhood. The bare connection inherits this;
...
The non-locality that blocks the bare connection (\S\ref{sec:link}) is, moreover,
not an accident of this construction. For \emph{any} connection rule on the
sprinkling that is a Lorentz-invariant function of pair (or neighbourhood)
invariants, the expected valency diverges: by a Campbell--Mecke argument the
marginal connection probability can depend only on the proper time $\Delta\tau$, so
the integral over neighbours factorises through the orbit at fixed
$\Delta\tau$---the boost hyperboloid, whose volume is infinite because the Lorentz
group is non-compact. For the covering relation this is the familiar non-locality of
causal-set links; the observation here is that it is not specific to links but
follows for every invariant rule. The smeared character of every workable causal-set
operator---including the gauge operator of \S\ref{sec:operator}---is the price of
that structural fact, not a technical choice.
...
content without magnetising; the low-curvature map is closed. The contribution is not a photon but a map: a new
gauge operator, a chain of pre-registered measurements that relocate the obstruction step
by step, and a single, concrete open construction---a Lorentz-invariant spacelike
$2$-cell on the causal order at low curvature---on which an emergent photon in this
framework now demonstrably depends.
